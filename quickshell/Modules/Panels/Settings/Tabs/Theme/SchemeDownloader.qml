...
Popup {
  ...
  function getSchemeColor(schemeName, colorKey) {
    ...
    var defaults = {
      "cSurface": Theme.cSurfaceVariant,
      "cPrimary": Theme.cPrimary,
      "cSecondary": Theme.cSecondary,
      "cTertiary": Theme.cTertiary,
      "cError": Theme.cError,
      "cOnSurface": Theme.cOnSurfaceVariant
    };
    return defaults[colorKey] || Theme.cOnSurfaceVariant;
  }
  ...
  function downloadSchemeFiles(schemeName, files) {
    ...
    var targetDir = ThemeService.downloadedSchemesDirectory + "/" + schemeName;
  ...
  function isSchemeInstalled(schemeName) {
    // Check if scheme already exists in ThemeService
    for (var i = 0; i < ThemeService.schemes.length; i++) {
      var path = ThemeService.schemes[i];
  ...
  function isSchemeDownloaded(schemeName) {
    ...
    for (var i = 0; i < ThemeService.schemes.length; i++) {
      var path = ThemeService.schemes[i];
      if ((path.indexOf("/" + schemeName + "/") !== -1 || path.indexOf("/" + schemeName + ".json") !== -1) && path.indexOf(ThemeService.downloadedSchemesDirectory) !== -1) {
        return true;
      }
    }
    return false;
  ...
  function deleteScheme(schemeName) {
    ...
    var deletedSchemeDisplayName = ThemeService.getBasename(schemeName);
    ...
    deleteProcess.exited.connect(function (exitCode) {
      if (exitCode === 0) {
        ...
        if (needsReset) {
          ...
          // Clear the setting immediately so ThemeService won't try to apply the deleted scheme
          Settings.data.colorSchemes.predefinedScheme = "CrawlDS (default)";
          // Apply the default scheme immediately
          ThemeService.setPredefinedScheme("CrawlDS (default)");
        }
  ...
  Connections {
    ...
    function onScanningChanged() {
      ...
      if (!ThemeService.scanning && pendingApplyScheme !== "") {
  ...
  Timer {
    ...
    onTriggered: {
      if (schemeName !== "") {
        ...
        ThemeService.setPredefinedScheme(schemeName);
        schemeName = "";
      }
    }
  }
      ...
      ColumnLayout {
        ...
        Repeater {
          ...
          Rectangle {
            ...
            border.color: hoverHandler.hovered ? root.getSchemeColor(schemeName, "cPrimary") : Theme.cOutline
            ...
            RowLayout {
              ...
              property var colorKeys: ["cPrimary", "cSecondary", "cTertiary", "cError"]
