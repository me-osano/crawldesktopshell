...
Item {
  ...
  Rectangle {
    ...
    anchors.centerIn: crawlBattery
    width: root.isBarVertical ? root.capsuleHeight : crawlBattery.width + Style.margin2S
    height: root.isBarVertical ? crawlBattery.height + Style.margin2S : root.capsuleHeight
    radius: Math.min(Style.radiusL, width / 2)
    color: graphicMouseArea.containsMouse ? Theme.cHover : Style.capsuleColor
  ...
  CrawlBattery {
    ...
    baseColor: graphicMouseArea.containsMouse ? Theme.cOnHover : Theme.cOnSurface
    textColor: graphicMouseArea.containsMouse ? Theme.cHover : Theme.cSurface
  }
  ...
  MouseArea {
    ...
    onClicked: mouse => {
                 if (mouse.button === Qt.RightButton) {
                   PanelService.showContextMenu(contextMenu, crawlBattery, screen);
                 } else {
                   openBatteryPanel();
                 }
               }
  ...
  BarPill {
    ...
    customBackgroundColor: root.isCharging ? Theme.cPrimary : ((root.isLowBattery || root.isCriticalBattery) ? Theme.cError : "transparent")
    customTextIconColor: root.isCharging ? Theme.cOnPrimary : ((root.isLowBattery || root.isCriticalBattery) ? Theme.cOnError : "transparent")
