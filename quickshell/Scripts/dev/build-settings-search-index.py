...
def get_subtab_info(parent_tab_file: Path) -> tuple[list[str], list[str | None]]:
    ...
    # Extract CrawlTabButton labels in order from CrawlTabBar
    labels = []
    in_tabbar = False
    tabbar_depth = 0
    ...
    for line in content.splitlines():
        ...
        if not in_tabbar:
            if re.match(r"CrawlTabBar\s*\{", stripped):
                in_tabbar = True
                tabbar_depth = 1
                continue
            continue
        ...
        # Match text: "..." inside CrawlTabButton
        m = re.search(r'text:\s*"([^"]+)"', stripped)
        if m:
            labels.append(m.group(1))

    # Extract subtab component names from CrawlTabView
    subtabs = []
    in_tabview = False
    tabview_depth = 0
    ...
    for line in content.splitlines():
        ...
        if not in_tabview:
            if re.match(r"CrawlTabView\s*\{", stripped):
                in_tabview = True
                tabview_depth = 1
                continue
            continue
...
def resolve_tab_info(
    ...
    # Determine sub-tab index and label from parent tab's CrawlTabBar/CrawlTabView
