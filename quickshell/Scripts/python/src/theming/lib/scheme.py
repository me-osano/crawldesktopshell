...
Input format (14 colors):
    cPrimary, cOnPrimary, cSecondary, cOnSecondary, cTertiary, cOnTertiary,
    cError, cOnError, cSurface, cOnSurface, cSurfaceVariant, cOnSurfaceVariant,
    cOutline, cHover
...
def expand_predefined_scheme(scheme_data: dict[str, str], mode: ThemeMode) -> dict[str, str]:
    ...
    Args:
        scheme_data: Dictionary with keys like cPrimary, cSecondary, etc.
        mode: "dark" or "light"
    ...
    # Parse input colors
    primary = _hex_to_color(scheme_data["cPrimary"])
    on_primary = _hex_to_color(scheme_data["cOnPrimary"])
    secondary = _hex_to_color(scheme_data["cSecondary"])
    on_secondary = _hex_to_color(scheme_data["cOnSecondary"])
    tertiary = _hex_to_color(scheme_data["cTertiary"])
    on_tertiary = _hex_to_color(scheme_data["cOnTertiary"])
    error = _hex_to_color(scheme_data["cError"])
    on_error = _hex_to_color(scheme_data["cOnError"])
    surface = _hex_to_color(scheme_data["cSurface"])
    on_surface = _hex_to_color(scheme_data["cOnSurface"])
    surface_variant = _hex_to_color(scheme_data["cSurfaceVariant"])
    on_surface_variant = _hex_to_color(scheme_data["cOnSurfaceVariant"])
    outline_raw = _hex_to_color(scheme_data["cOutline"])
    shadow = _hex_to_color(scheme_data.get("cShadow", scheme_data["cSurface"]))
    ...
    # Generate surface containers using cSurfaceVariant as the middle container
    ...
    # surface_container = cSurfaceVariant (direct assignment)
