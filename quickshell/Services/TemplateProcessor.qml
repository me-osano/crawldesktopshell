...
Singleton {
  ...
  */
  ...
  function executePredefinedScheme(schemeData, mode, wallpaperPath) {
    ...
    // 3. Extract mode-specific scheme data for IPC
    const modeData = schemeData[mode] || schemeData;

    // 4. Call daemon IPC to expand the 14-color scheme to full 48-color palette
    //    (replaces Python's expand_predefined_scheme which is now in Rust)
    CrawlService.sendRequest("ThemeGenerateFromPredefined", {
      scheme_json: JSON.stringify(modeData),
      mode: mode
    }, function(result, error) {
      if (error) {
        Logger.e("TemplateProcessor", "Daemon scheme expansion failed:", error);
        ToastService.showError("Scheme Generation", "Failed to expand predefined scheme");
        return;
      }

      // result is the expanded 48-color palette: {"primary": "#...", "on_primary": "#...", ...}
      const expandedPalette = result;

      // 5. Write expanded palette and TOML config, then run Python for template rendering
      ...
      // Write expanded palette JSON (48-color format, already expanded by daemon)
      script += `cat > '${schemeJsonPathEsc}' << '${schemeDelimiter}'\n`;
      script += JSON.stringify(expandedPalette, null, 2) + "\n";
      ...
      // Run Python template processor with expanded palette as positional arg
      // (Python reads .json positional arg as flat palette, no expansion needed)
      // Pass --config for template definitions and --default-mode for "default" resolution
      script += `python3 "${templateProcessorScript}" '${schemeJsonPathEsc}' --config '${configPathEsc}' --default-mode ${mode}\n`;
  ...
  */
  ...
  /**
  * Generate terminal themes via daemon IPC (replaces Python template-processor)
  */
  function handleTerminalThemesGenerate(schemeData, mode, homeDir) {
    ...
    // Extract mode-specific scheme data (terminal section + UI colors)
    const modeData = schemeData[mode] || schemeData;
    ...
    // Call daemon IPC to generate terminal themes
    CrawlService.sendRequest("ThemeGenerateTerminal", {
      scheme_json: JSON.stringify(modeData),
      outputs_json: JSON.stringify(terminalOutputs)
    }, function(result) {
      if (!result || result.error) {
        Logger.e("TemplateProcessor", "Daemon terminal generation failed:", result ? result.error : "no result");
        ToastService.showError("Terminal Themes", "Generation failed");
        return;
      }
      Logger.d("TemplateProcessor", "Terminal themes generated via daemon");

      // Run post-hooks for enabled terminals
      TemplateRegistry.terminals.forEach(terminal => {
                                           if (isTemplateEnabled(terminal.id)) {
                                             Quickshell.execDetached(["sh", "-c", terminal.postHook]);
                                           }
                                         });
    });
  }
  ...
  */
  ...
  function getTerminalColorsTemplate(terminal, mode) {
    ...
    for (let i = 0; i < ThemeService.schemes.length; i++) {
      const schemeJsonPath = ThemeService.schemes[i];
    ...
    const downloadedPathHyphen = `${ThemeService.downloadedSchemesDirectory}/${colorScheme}/${relativePathHyphen}`;
    const downloadedPathSpace = `${ThemeService.downloadedSchemesDirectory}/${colorScheme}/${relativePathSpace}`;
    const preinstalledPathHyphen = `${ThemeService.schemesDirectory}/${colorScheme}/${relativePathHyphen}`;
    const preinstalledPathSpace = `${ThemeService.schemesDirectory}/${colorScheme}/${relativePathSpace}`;
  ...
  function buildUserTemplateCommandForPredefined(schemeData, mode, wallpaperPath) {
    ...
    // Reuse the expanded palette JSON already written by executePredefinedScheme()
    const schemeJsonPathEsc = schemeJsonPath.replace(/'/g, "'\\''");

    let script = "\n# Execute user templates with predefined scheme colors\n";
    script += `if [ -f '${userConfigPath}' ]; then\n`;
    // Pass expanded palette JSON as positional arg (already expanded by daemon IPC)
    // No --scheme flag needed — Python reads .json positional arg as flat palette
    // Don't pass --mode so user templates get both dark and light colors
    // Pass --default-mode so "default" in templates resolves to the current theme mode
    script += `  python3 "${templateProcessorScript}" '${schemeJsonPathEsc}' --config '${userConfigPath}' --default-mode ${mode}\n`;
    script += "fi";
