...
// here any CrawlText properties can be used
}
maxWidth: 200
text: "Some long long long text"
scrollMode: CrawlScrollText.ScrollMode.Always
}
*/
...
Item {
  ...
  default property Component delegate: CrawlText {
    pointSize: Style.fontSizeS
  }
  ...
  property int scrollMode: CrawlScrollText.ScrollMode.Never
  ...
  property int state: CrawlScrollText.ScrollState.None
  ...
  function resetState() {
    root.state = CrawlScrollText.ScrollState.None;
    scrollContainer.x = 0;
    scrollTimer.restart();
    root.updateState();
  }
  ...
  Timer {
    ...
    onTriggered: {
      root.state = CrawlScrollText.ScrollState.Scrolling;
      root.updateState();
    }
  }
  ...
  Timer {
    ...
    repeat: true
    running: root.state === CrawlScrollText.ScrollState.Scrolling
    onTriggered: {
  ...
  function ensureReset() {
    if (state === CrawlScrollText.ScrollState.Scrolling)
      state = CrawlScrollText.ScrollState.Resetting;
  }

  function updateState() {
    if (contentWidth <= root.maxWidth || scrollMode === CrawlScrollText.ScrollMode.Never) {
      state = CrawlScrollText.ScrollState.None;
      return;
    }
    if (scrollMode === CrawlScrollText.ScrollMode.Always) {
      if (hoverArea.containsMouse) {
        ensureReset();
      } else {
        scrollTimer.restart();
      }
    } else if (scrollMode === CrawlScrollText.ScrollMode.Hover) {
      if (hoverArea.containsMouse || forcedHover)
        state = CrawlScrollText.ScrollState.Scrolling;
      else
        ensureReset();
    }
  }
  ...
  RowLayout {
    ...
    Loader {
      ...
      sourceComponent: root.delegate
      Layout.fillHeight: true
      visible: root.state !== CrawlScrollText.ScrollState.None
      onLoaded: {
        this.item.text = root.text;
    ...
    NumberAnimation on x {
      running: root.state === CrawlScrollText.ScrollState.Resetting
      to: 0
