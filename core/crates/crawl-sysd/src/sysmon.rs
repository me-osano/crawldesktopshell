...
//! SysmonEvents. Also exposes synchronous query functions for the IPC router.

use crate::config::SysmonConfig;
use crawl_ipc::{
    events::{CrawlEvent, SysmonEvent},
    types::{CpuStatus, DiskStatus, GpuStatus, LoadAvg, MemStatus, NetTraffic},
};
...
impl SysmonState {
    ...
    fn mem_changed(&self, new: &MemStatus, threshold: f32) -> bool {
        let old_pct = self
            ...
            .map(|m| {
                if m.total_kb > 0 {
                    m.used_kb as f32 / m.total_kb as f32 * 100.0
                } else {
                    0.0
                }
            })
...
pub async fn run(cfg: SysmonConfig, tx: broadcast::Sender<CrawlEvent>) -> anyhow::Result<()> {
    info!(
        "crawl-sysmon starting (interval={}ms)",
    ...
    loop {
        ...
        if state.cpu_changed(&cpu, cfg.cpu_change_threshold) {
            let _ = tx.send(CrawlEvent::Sysmon(SysmonEvent::CpuUpdate {
                cpu: cpu.clone(),
            }));
        ...
        if state.mem_changed(&mem, cfg.mem_change_threshold) {
            let _ = tx.send(CrawlEvent::Sysmon(SysmonEvent::MemUpdate {
                mem: mem.clone(),
            }));
        ...
        if used_pct > cfg.mem_pressure_threshold && !state.mem_pressure_sent {
            let _ = tx.send(CrawlEvent::Sysmon(SysmonEvent::MemPressure {
        ...
        if state.net_changed(&traffic, cfg.net_change_threshold) {
            let _ = tx.send(CrawlEvent::Sysmon(SysmonEvent::NetUpdate {
        ...
        if state.gpu_changed(&gpu) {
            if let Some(ref gpu) = gpu {
                let _ = tx.send(CrawlEvent::Sysmon(SysmonEvent::GpuUpdate {
                    gpu: gpu.clone(),
                }));
            }
            state.last_gpu = gpu;
...
pub fn build_cpu_status(sys: &System) -> CpuStatus {
    ...
    let aggregate = if cores.is_empty() {
        0.0
    } else {
    ...
    fn sample(&mut self) -> NetTraffic {
        ...
        for (iface, (rx, tx)) in current.iter() {
            ...
            if let Some((prev_rx, prev_tx)) = self.prev.get(iface) {
                rx_bps =
                    rx_bps.saturating_add((rx.saturating_sub(*prev_rx)) * 1000 / elapsed_ms as u64);
                tx_bps =
                    tx_bps.saturating_add((tx.saturating_sub(*prev_tx)) * 1000 / elapsed_ms as u64);
            }
        }
