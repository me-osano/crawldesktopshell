...
pub fn detect_current_channel(iface: &str) -> Option<u32> {
    let out = std::process::Command::new("sh")
        .args([
            "-c",
            &format!("iw dev {} info | awk '/channel/ {{print $2}}'", iface),
        ])
        .output()
        .ok()?;
    let freq: u32 = String::from_utf8_lossy(&out.stdout).trim().parse().ok()?;
    util::frequency_channel(freq)
}
...
pub async fn detect_wifi_phy(iface: &str) -> Result<String, NetError> {
    let out = run_shell(&format!(
        "iw dev {} info | awk '/wiphy/ {{print $2}}' | head -1",
        iface
    ))
    .await?;
...
#[expect(dead_code)]
pub async fn run_cmd(cmd: &str, args: &[&str]) -> Result<String, NetError> {
    let output = Command::new(cmd)
        .args(args)
        .output()
        .await
        .map_err(|_e| NetError::Unavailable)?
        .stdout;
    Ok(String::from_utf8_lossy(&output).to_string())
}

#[expect(dead_code)]
pub async fn run_cmd_bg(cmd: &str, args: Vec<String>) -> Result<(), NetError> {
    Command::new(cmd)
        .args(&args)
        .stdin(Stdio::null())
        .kill_on_drop(true)
...
pub async fn run_shell(script: &str) -> Result<String, NetError> {
...
pub async fn nm_mark_unmanaged(iface: &str) -> Result<(), NetError> {
    run_shell(&format!(
        "nmcli device set {} managed no 2>/dev/null || true",
        iface
    ))
    .await?;
    Ok(())
...
#[expect(dead_code)]
pub async fn nm_restore_managed(iface: &str) -> Result<(), NetError> {
    run_shell(&format!(
        "nmcli device set {} managed yes 2>/dev/null || true",
        iface
    ))
    .await?;
    Ok(())
...
pub async fn setup_ip_forward(ap_iface: &str, upstream_iface: &str) -> Result<(), NetError> {
...
pub fn teardown_nat(ap_iface: &str, upstream_iface: &str) {
    let _ = std::process::Command::new("sh")
        .args(["-c", &format!(
            ...
            upstream_iface, ap_iface, upstream_iface, ap_iface, ap_iface
        )])
        .output();
}
