...
use crate::network::dbus::{
    NM_DEVICE_TYPE_ETHERNET, NMDeviceProxy, NMDeviceWiredProxy, NMIP4ConfigProxy, NMIP6ConfigProxy,
    NetworkManagerProxy,
};
use crate::network::{NetError, sysfs, util};
use crawl_ipc::types::{ActiveEthernetDetails, EthernetInterface};
...
pub async fn list_ethernet_interfaces(
    ...
    for path in nm.get_devices().await? {
        ...
        let ip4 = if ip4_raw == 0 {
            ...
            Some(std::net::Ipv4Addr::from(ip4_raw).to_string())
        };
...
pub async fn refresh_ethernet_details(
    ...
    if let Ok(wired) = NMDeviceWiredProxy::builder(conn)
        .path(eth_device.clone())?
        .build()
        .await
    {
    ...
    if details.speed.is_none() && !details.ifname.is_empty() {
        if let Some(sysfs_speed) = sysfs::read_sysfs_speed(&details.ifname) {
            ...
            if let Ok(addr) = ip4.address_data().await {
                if let Some(first) = addr.first() {
                    if let Some(value) = first.get("address") {
                        details.ipv4 = util::owned_value_str(value);
                    }
                }
            }
...
pub async fn get_ethernet_details(
    interface: Option<&str>,
) -> Result<ActiveEthernetDetails, NetError> {
    let conn = Connection::system().await?;
    let nm = NetworkManagerProxy::new(&conn).await?;
...
pub async fn connect_ethernet(interface: Option<&str>) -> Result<String, NetError> {
    ...
    let (eth_device, iface_name) = match interface {
        ...
        None => find_first_ethernet_device(&conn, &devices)
            .await
    ...
    nm.add_and_activate_connection(settings, eth_device, root)
        .await?;
    Ok(iface_name)
}
...
pub async fn disconnect_ethernet(interface: Option<&str>) -> Result<String, NetError> {
    ...
    if active.as_str() == "/" {
        return Err(NetError::NotFound(format!(
            "no active connection for '{iface_name}'"
        )));
    }
...
fn build_ethernet_settings(
    interface: &str,
) -> HashMap<String, HashMap<String, zbus::zvariant::OwnedValue>> {
    let mut connection = HashMap::new();
    connection.insert("id".to_string(), util::owned_value(interface.to_string()));
    connection.insert(
        "type".to_string(),
        util::owned_value("802-3-ethernet".to_string()),
    );
