...
pub use crate::config::NetworkConfig as Config;
pub use crate::{Scheduler, ShouldRun};
pub use crawl_ipc::{
...
pub use tokio::time::{Duration, interval};
pub use tracing::info;

use dbus::{
    NM_DEVICE_TYPE_ETHERNET, NM_DEVICE_TYPE_WIFI, NMAccessPointProxy, NMDeviceProxy,
    NMDeviceWirelessProxy, NMIP6ConfigProxy, NMSettingsConnectionProxy, NMSettingsProxy,
    NetworkManagerProxy,
};
...
pub async fn run_with_scheduler(
    ...
    if let Some(ref sched) = scheduler {
        sched
            .register(task_id_fast, Duration::from_secs(5), 0.1)
            .await;
        sched
            .register(task_id_slow, Duration::from_secs(30), 0.15)
            .await;
    }

    info!("crawl-network starting (scheduler={})", scheduler.is_some());
    ...
    if cfg.auto_enable {
        if nm.networking_enabled().await.unwrap_or(false) != true {
            let _ = nm.set_networking_enabled(true).await;
        }
        if nm.wireless_enabled().await.unwrap_or(false) != true {
            let _ = nm.set_wireless_enabled(true).await;
        }
        info!("auto-enabled networking and wifi");
    }
    ...
    loop {
        tokio::select! {
            ...
            _ = slow_ticker.tick() => {
                ...
                if let Ok(status) = hotspot::hotspot_status(Some(&conn)).await {
...
pub async fn connect_wifi(cfg: wifi::WifiConnectConfig) -> Result<(), NetError> {
    wifi::connect_wifi(cfg).await
}
...
pub async fn delete_wifi_connection(ssid: &str) -> Result<(), NetError> {
            ...
            if let Ok(settings_map) = conn_obj.get_settings().await {
                if let Some(wireless) = settings_map.get("802-11-wireless") {
                    if let Some(value) = wireless.get("ssid") {
                        if let Some(ssid_value) = ssid_from_value(value) {
                            if ssid_value == ssid {
                                conn_obj.delete().await?;
                                return Ok(());
                            }
                        }
                    }
    ...
    Err(NetError::NotFound(format!(
        "wifi connection '{ssid}' not found"
    )))
}

pub use ethernet::{connect_ethernet, disconnect_ethernet, get_ethernet_details, list_ethernet};
pub use hotspot::hotspot_status;
pub use wifi::WifiConnectConfig;
...
async fn refresh_snapshot(
    ...
    for path in nm.get_devices().await? {
        ...
        let ip4 = if ip4_raw == 0 {
            None
        } else {
            Some(Ipv4Addr::from(ip4_raw).to_string())
        };
        let mac = dev.hw_address().await.ok();
        let device_type = dev.device_type().await.unwrap_or(0);

        let ip6_path = dev.ip6_config().await.ok();
        let ip6_addrs = if let Some(ref path) = ip6_path {
            if path.as_str() != "/" {
                if let Ok(ip6) = NMIP6ConfigProxy::builder(conn)
                    .path(path.clone())?
                    .build()
                    .await
                {
                    if let Ok(addr) = ip6.address_data().await {
                        addr.iter()
                            .filter_map(|e| e.get("address").and_then(|v| util::owned_value_str(v)))
                            .collect()
                    } else {
                        Vec::new()
                    }
