...
use crawl_ipc::events::{CrawlEvent, ProcEvent};
...
pub async fn run(cfg: ProcConfig, tx: broadcast::Sender<CrawlEvent>) -> anyhow::Result<()> {
    info!(
        "crawl-proc starting (top: {}ms, full: {}ms, include_cmd: {})",
        cfg.top_interval_ms, cfg.full_interval_ms, cfg.include_cmd
    );

    // Seed global cache with configured options, then do initial fill
    {
        let mut global = PROCESS_CACHE.lock().unwrap();
        *global = ProcessCache::new(cfg.include_cmd);
        global.full_refresh();
    }
    ...
    loop {
        ...
        if now.duration_since(last_full) >= full_interval {
            ...
            drop(global);
            last_full = now;
            debug!("full process scan done, {len} processes");
        }
        ...
        if now.duration_since(last_top) >= top_interval {
            ...
            let top_mem;
            {
                let global = PROCESS_CACHE.lock().unwrap();
                top_cpu = global.top_by_cpu(10);
                top_mem = global.top_by_mem(10);
            }
...
pub fn kill_process(pid: u32, force: bool) -> Result<(), ProcError> {
    ...
    process
        .kill_with(signal)
        .ok_or_else(|| ProcError::SignalFailed(format!("kill({pid}) failed")))?;
    Ok(())
}
...
static PROCESS_CACHE: Lazy<Mutex<ProcessCache>> =
    Lazy::new(|| Mutex::new(ProcessCache::new(false)));
...
mod cache {
    ...
    impl ProcessCache {
        ...
        pub fn refresh_top(&mut self) {
            ...
            self.by_pid
                .retain(|pid, _| sys.process(Pid::from_u32(*pid)).is_some());
